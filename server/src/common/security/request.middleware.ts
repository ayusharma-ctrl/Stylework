import { ForbiddenException, HttpException, Injectable, ServiceUnavailableException, UnsupportedMediaTypeException } from '@nestjs/common';
...
export class RequestMiddleware {
  ...
  constructor(private readonly limiter:RateLimiter,private readonly state:RuntimeState,private readonly telemetry:Telemetry) {}
  use = async (req:ApiRequest,res:Response,next:NextFunction) => {
    ...
    try {
      ...
      if(req.path==='/signin') await this.limiter.consume('signin-ip',ip,config.SIGNIN_IP_LIMIT,60000,res);
        ...
        await this.limiter.consume(read?'read-ip':'write-ip',ip,read?config.READ_IP_LIMIT:config.WRITE_IP_LIMIT,60000,res);
      }
      next();
    } catch(error) {
      const status=error instanceof HttpException?error.getStatus():503;
      const detail=error instanceof HttpException?error.getResponse():null;
      const body=typeof detail==='object'?detail as any:{message:typeof detail==='string'?detail:'Service temporarily unavailable'};
      if(status===503)res.setHeader('Retry-After','2');
      res.status(status).json({error:{code:body.code||'REQUEST_REJECTED',message:body.message},meta:{requestId:req.requestId}});
    }
  };
}
