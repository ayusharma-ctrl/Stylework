...
export async function migrate(db: Sequelize) {
  const runner = new Umzug({ migrations: [{ name: '001-initial', up: () => up(db) },{ name:'002-webhook-credentials',up:()=>credentials(db) }], context: db, storage: new SequelizeStorage({ sequelize: db }), logger: console });
