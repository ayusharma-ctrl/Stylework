...
export class LeadStatusService {
 constructor(private readonly db:DatabaseService,private readonly audit:AuditService,private readonly counters:CountersService,private readonly outbox:OutboxService,private readonly repository:LeadsRepository){}
 async change(id:string,input:ChangeLeadStatus,principal:Principal,requestId:string){
  await this.db.sequelize.transaction(async transaction=>{
   ...
   if(lead.version!==input.expectedVersion)throw new ConflictException({code:'VERSION_CONFLICT',message:'Lead changed; refresh before updating'});
   ...
   await this.audit.record(transaction,{entityId:lead.id,leadId:lead.id,entityType:'lead',type:'STATUS_CHANGED',actorId:principal.id,actor:{kind:'user',label:principal.email},summary:'Status changed from '+previous!.name+' to '+status.name,before:{status:{id:previous!.id,name:previous!.name}},after:{status:{id:status.id,name:status.name}},requestId});
