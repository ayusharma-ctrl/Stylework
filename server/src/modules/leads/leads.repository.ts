...
function serialize(row: LeadView): LeadView {
  return {
    ...row,
...
export class LeadsRepository {
  ...
  async find(id: string) {
    const rows = await this.db.sequelize.query<LeadView>(
      'SELECT ' + columns + ' FROM leads l JOIN statuses s ON s.id=l.status_id WHERE l.id=:id',
  ...
  async list(input: LeadQuery) {
    const page = paging(input),
      column = sortColumns[input.sort];
    const where = ['l.created_at<=:asOf'],
      replacements: Record<string, any> = { asOf: page.asOf, limit: page.limit + 1 };
    if (input.search) {
      where.push("l.search_text LIKE :search ESCAPE '\\'");
      replacements.search = '%' + escapeLike(input.search.toLowerCase()) + '%';
    }
    if (input.statusIds?.length) {
      where.push('l.status_id IN (:statusIds)');
      replacements.statusIds = input.statusIds;
    }
    if (input.source) {
    ...
    if (page.cursor) {
      where.push('(' + column + ',l.id) ' + (asc ? '>' : '<') + ' (:value,:id)');
      replacements.value = page.cursor.value;
      replacements.id = page.cursor.id;
    }
    const order = asc ? 'ASC' : 'DESC';
    const rows = await this.db.sequelize.query<LeadView & { cursorValue: string }>(
      'SELECT ' +
        columns +
        ', ' +
        column +
        '::text AS "cursorValue" FROM leads l JOIN statuses s ON s.id=l.status_id WHERE ' +
        where.join(' AND ') +
        ' ORDER BY ' +
        column +
        ' ' +
    ...
    return connection(
      rows.map(({ cursorValue, ...row }) => serialize(row)),
      page,
