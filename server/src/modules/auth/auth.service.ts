...
export class AuthService {
  ...
  async signin(email: string) {
    ...
    const id = randomUUID(),
      now = Date.now(),
      accessExpiresAt = new Date(now + 15 * 60000),
      refreshExpiresAt = new Date(now + 7 * 86400000);
    const accessToken = this.token(user.id, id, 'access', accessExpiresAt),
      refreshToken = this.token(user.id, id, 'refresh', refreshExpiresAt);
  ...
  async authenticate(accessToken: string, refreshToken: string, response?: Response): Promise<Principal> {
    const access = this.verify(accessToken, 'access'),
      refresh = this.verify(refreshToken, 'refresh');
