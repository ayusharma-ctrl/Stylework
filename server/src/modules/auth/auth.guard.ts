...
export function httpContext(context: ExecutionContext): { req:ApiRequest; res:Response } {
  return context.getType<string>()==='graphql' ? GqlExecutionContext.create(context).getContext() : { req:context.switchToHttp().getRequest(),res:context.switchToHttp().getResponse() };
}
@Injectable()
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext) {
    ...
    const header=req.headers.authorization, refresh=req.headers['x-refresh-token'];
    if (!header?.startsWith('Bearer ') || typeof refresh!=='string') throw new UnauthorizedException('Access and refresh token headers are required');
