...
export class WebhooksService {
 ...
 private async overloaded() {
  if(Date.now()-this.checked>1000) {
   this.checking ||= this.db.sequelize.query<{count:number}>("SELECT count(*)::int AS count FROM (SELECT id FROM webhook_receipts WHERE state='pending' LIMIT :limit) q",{type:QueryTypes.SELECT,replacements:{limit:config.MAX_PENDING_EVENTS}}).then(rows=>{this.backlog=rows[0]!.count;this.checked=Date.now();}).finally(()=>{this.checking=undefined;});
   await this.checking;
  }
 ...
 async accept(payload:WebhookDto,requestId:string,credentialId:string) {
  ...
  return this.db.sequelize.transaction(async transaction=>{
   ...
   if(existing) {
    if(existing.payloadHash!==payloadHash)throw new ConflictException({code:'EVENT_ID_CONFLICT',message:'This event ID was already used with a different payload'});
    return {...this.repository.safe(existing),duplicate:true};
   }
   if(overloaded)throw new ServiceUnavailableException({code:'INTAKE_OVERLOADED',message:'Intake backlog is full; retry this event later'});
   const receipt=await Receipt.create({source:'meta',eventId:payload.eventId,payloadHash,payload,requestId},{transaction});
   await Outbox.create({kind:'process',receiptId:receipt.id,payload:{receiptId:receipt.id,requestId}},{transaction});
   return {...this.repository.safe(receipt),duplicate:false};
  });
 }
 async outcome(eventId:string) {const receipt=await this.repository.find(eventId);if(!receipt)throw new NotFoundException('Webhook event not found');return this.repository.safe(receipt);}
}
