...
(async()=>{
 ...
 try{
  ...
  const signed=await fetch(base+'/signin',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:'query-smoke@example.test'})});
  const {data:profile}=await signed.json();if(!profile)throw Error('Sign-in failed');
  ...
  const query='query Leads($input: LeadQueryInput) { leads(input:$input) { nodes { id fullName status { id name } } pageInfo { startCursor endCursor hasNextPage hasPreviousPage } } }';
  async function gql(document,variables={}){const r=await fetch(base+'/graphql',{method:'POST',headers,body:JSON.stringify({query:document,variables})});return r.json();}
  ...
  const tooMany=await gql('{'+Array.from({length:21},(_,i)=>'a'+i+': leads(input:{first:1}) { nodes { id } }').join(' ')+'}');
