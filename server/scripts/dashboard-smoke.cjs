...
(async () => {
  ...
  try {
    ...
    const profile = (
      await (
        await fetch(base + '/signin', {
          ...
          body: JSON.stringify({ email: 'dashboard-smoke@example.test' }),
        })
      ).json()
    ).data;
    const headers = {
    ...
    async function snapshot() {
      const deadline = setTimeout(() => abort.abort(), 7000);
      try {
        for (;;) {
          ...
          if (split >= 0) {
            ...
            if (chunk.done) throw Error('Stream closed');
    ...
    assert.equal(initial.metrics.length, 4);
    assert.equal(initial.timezone, 'Asia/Kolkata');
    assert.ok(initial.statuses.length >= 5);
    const lead = (await (await fetch(base + '/leads?first=1', { headers })).json()).data.nodes[0];
    const target = profile.statuses.find((s) => !s.archivedAt && s.id !== lead.status.id);
    const changed = await fetch(base + '/leads/' + lead.id + '/status', {
      method: 'PATCH',
      headers,
      body: JSON.stringify({ statusId: target.id, expectedVersion: lead.version }),
    });
    ...
    assert.notEqual(initial.revision, next.revision);
    assert.equal(
      next.statuses.find((s) => s.id === target.id).value,
      initial.statuses.find((s) => s.id === target.id).value + 1,
    );
    abort.abort();
