...
(async () => {
 ...
 try {
  ...
  const signin=await fetch(base+'/signin',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:'smoke@example.test'})});
  if(!signin.ok) throw new Error('Sign-in failed: '+signin.status);
  const envelope=await signin.json(); const profile=envelope.data||envelope;
  const headers={Authorization:'Bearer '+profile.tokens.accessToken,'X-Refresh-Token':profile.tokens.refreshToken,'Content-Type':'application/json'};
