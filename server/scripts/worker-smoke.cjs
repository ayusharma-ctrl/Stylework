...
(async () => {
  ...
  try {
    ...
    let current;
    for (let i = 0; i < 100; i++) {
      current = await Receipt.findByPk(receipt.id);
      if (current.state !== 'pending') break;
      await new Promise((r) => setTimeout(r, 100));
    }
    if (current.state !== 'processed')
      throw Error('Processing did not complete: ' + current.state + ' ' + current.errorCode);
    await Promise.all([
      app.get(LeadProcessor).process(receipt.id),
      app.get(LeadProcessor).process(receipt.id),
    ]);
    if ((await Activity.count({ where: { leadId: current.leadId, type: 'LEAD_CREATED' } })) !== 1)
      throw Error('Duplicate lead audit');
    console.log('PASS worker smoke: durable processing and idempotent redelivery');
  } finally {
    await app.get(JobsService).stop();
    await app.close();
  }
})().catch((error) => {
