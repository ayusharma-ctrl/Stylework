...
(async () => {
  ...
  const keys = app.get(WebhookCredentialsService),
    credential = await keys.create('Intake smoke');
  try {
    ...
    const payload = {
      ...
      data: { fullName: 'Intake smoke', email: 'intake@example.test' },
    };
    async function send(value, valid = true) {
      return fetch(base + '/webhook/meta-lead', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Webhook-Key': valid ? credential.key : 'invalid' },
    ...
    if (
      results.filter((r) => r.status === 202).length !== 1 ||
      results.filter((r) => r.status === 200).length !== 9
    )
      throw Error('Duplicate acceptance mismatch');
    if ((await send({ ...payload, data: { ...payload.data, fullName: 'Conflicting' } })).status !== 409)
      throw Error('Conflict rejection failed');
