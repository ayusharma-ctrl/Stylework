...
## Current
September 25 revision complete: two IP quotas, frontend port 5173, embedded jobs in one backend process, and one free Render web service configured in server/render.yaml. Builds, 25 unit tests, 25 integration scenarios, both browser journeys and combined-runtime smoke passed.
## Remaining
...
- Removed read/write/user/email rate policies and RateGuard. REST shares API_RATE_LIMIT=120/minute/IP; the webhook endpoint shares WEBHOOK_RATE_LIMIT=60/minute/IP across keyed/manual calls. Regression tests cover cross-route/method/identity aggregation, forwarded-header spoofing with no trusted proxy, independent webhook allowance and case/trailing-slash handling.
- API imports JobsModule by default; RUN_WORKER=false retains the optional split-deployment path. Pool default 4 and worker concurrency 1 are explicit in Compose/Render. Jobs drain before Redis/PostgreSQL close. Reduced dispatch polling to once/second, recovery every 30 seconds and batches of 25.
- Frontend Docker host port and Vite strict port are 5173; internal Nginx stays 8080. Render Blueprint moved to server/render.yaml and contains one free web service, external datastore URLs and migrations-before-start (no paid pre-deploy hook). Free-tier sleep/cold starts and paused jobs are documented.
- Started the stopped local Docker engine without resetting volumes. Removed the obsolete standalone worker container. Both images built, all four running services became healthy, and smoke processed a webhook through lead/status/audit/SSE with only the API process. Runtime confirmed RUN_WORKER=true, pool 4, concurrency 1 and quotas 120/60.
- Backend 25 unit and 25 database/Redis integration tests passed; fixed a test helper to select meta receipts explicitly when event IDs exist in both source namespaces. Two Chromium journeys passed at localhost:5173. YAML parsed with one free service. No hosted deployment or large-data test performed. User edits in auth/common/Jest files are preserved; config formatting is kept outside the implementation commit.
