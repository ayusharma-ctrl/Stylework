...
Models live one per file under server/src/database/models. The former workspace_settings singleton is renamed app_settings by migration 003; configuration and concurrency locks are preserved without any tenant model. Manual intake uses the existing webhook route with both session tokens and normal mutation quotas, while external callers still use database-backed keys. Receipts persist trusted actor details and isolate manual/meta source IDs. Both date bounds use safe Sequelize replacement spacing and compare instants, not offset strings. API responses negotiate gzip/Brotli; frontend Nginx serves gzip assets; SSE and sign-in remain uncompressed.
